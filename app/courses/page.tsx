...
export default function CoursesPage() {
  ...
  const courses = [
    ...
    },
    {
      ...
      icon: <Server className="h-10 w-10 text-cyan-600" />,
      duration: "24 weeks",
      level: "Intermediate",
      category: "Cloud Computing",
      price: "₹35000",
    ...
    <MainLayout>
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
          <div>
            <h2 className="text-2xl font-bold mb-2 text-gray-900">Find Your Perfect Course</h2>
            <p className="text-gray-600">Browse our comprehensive catalog of professional courses</p>
          </div>
          <div className="relative w-full md:w-72">
            ...
            <Input
              ...
              className="w-full pl-8 border-gray-200 focus:border-emerald-500 focus:ring-emerald-500"
            />
          </div>
        </div>

        <Tabs defaultValue="all" className="w-full">
          <TabsList className="mb-8 flex flex-wrap h-auto bg-gray-100 p-1 rounded-lg">
            {courseCategories.map((category) => (
              <TabsTrigger
                key={category.id}
                value={category.id}
                className="mb-2 data-[state=active]:bg-white data-[state=active]:text-emerald-700 data-[state=active]:shadow-sm"
              >
                {category.label}
              </TabsTrigger>
            ))}
                      ...
                      <CardContent>
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          ...
                          <div>
                            ...
                            <p className="font-medium text-emerald-700">{course.price}</p>
                          </div>
                          <div>
